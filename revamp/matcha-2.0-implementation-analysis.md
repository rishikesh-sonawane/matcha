...
- F-14 (Low): no agent-reach → `doctor_snapshot()` None; **make sure `agent_reach_io` degrades to Matcha's own probes and logs a one-time hint** (`npm install -g @jackwener/opencli` already satisfied; `pip install agent-reach` optional). **RESOLVED Phase 1:** (a) Exa part — `backends/mcporter.py` reads mcporter config read-only (`MCPORTER_CONFIG` → `~/.mcporter/mcporter.json{,c}` → `<cwd>/config/mcporter.json`; never starts mcporter; never expands `imports` — credential boundary); `backends/exa.py` dual-syntax `mcporter call` (openclaw 0.8+ `key=value` first, legacy 0.7 DSL retry), `includeDomains` retry guard, error-envelope detection; Web Search degrades to DDGS when mcporter is absent (verified live: `exa_status()==off` → `ddgs`). (b) **`agent_reach_io.py` itself** — `doctor_snapshot()` (`agent-reach doctor --json`, TTL-cached 30s, credential-scrubbed, None-degrading) + one-time warning hint when agent-reach absent (verified live); `opencli_ready()` snapshot-first with own-probe fallback; `exa_search()` delegates to `backends/exa.py`; `gh_profile()` read-only (env tokens or hosts.yml; never `gh auth status`); `seed_ai_config()` borrows groq_api_key from `~/.agent-reach/config.yaml`. 31 hermetic tests.
...
- **Indeed backend caution:** `clis/indeed/` exists but its output shape is unverified — treat like F-07 (verify `opencli indeed search --help` first). **RESOLVED Phase 1:** shapes locked from `clis/indeed/search.js` + `utils.js`; adapter is **US-site** (§2). Naukri job-page extraction: the real posting page must be fetched via OpenCLI `web` adapter or `curl`; verify Naukri blocks headless. **RESOLVED Phase 1 (2026-08-06):** verified live that Naukri is a client-rendered Next.js RSC shell (plain requests → empty `jobDetails:[]`, no JSON-LD; internal `jobapi/v3` rejects unauthenticated calls 404/405) — the page is fetched via **Jina Reader render** (`r.jina.ai/<url>`, zero-config, same pattern as §8) with a direct GET + embedded JSON-LD/`__NEXT_DATA__` parse attempted first (wins if Naukri ever server-renders). `sources/naukri.py` backends `["job-page", "ddgs"]`: DDGS discovery → parallel fetch of real `job-listings-*` postings (cap 8, 12s, ≤4 workers, per-job isolation) → parse description/salary/experience/key skills/apply URL; expired postings redirect to search pages → detected + kept as snippets. 18 hermetic tests.
